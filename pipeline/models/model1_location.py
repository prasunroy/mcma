...
class LocationPredictor(nn.Module):
    ...
    def __init__(self, embeds_dim=512, latent_dim=64):
        super(LocationPredictor, self).__init__()
        self.frozen_features = FrozenFeatureExtractor()
        self.mutual_cross_attention = MutualCrossAttentionBlock(512, 512, embeds_dim)
        self.downsample = nn.Sequential(
            nn.Conv2d(embeds_dim, embeds_dim, 4, 2, 1, bias=False),
            nn.BatchNorm2d(embeds_dim),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d(output_size=(4, 4)),
            nn.Flatten()
        )
        self.fc_proj_encode = nn.Sequential(nn.Linear(embeds_dim*4*4, embeds_dim), nn.ReLU(inplace=True))
        self.fc_location_xy = nn.Sequential(nn.Linear(2, embeds_dim), nn.ReLU(inplace=True), nn.Linear(embeds_dim, embeds_dim), nn.ReLU(inplace=True))
    ...
    def estimate_shared_features(self, img, seg):
        i = self.frozen_features(img)
        s = self.frozen_features(seg)
        f = self.mutual_cross_attention(i, s)
        f = self.downsample(f)
        return self.fc_proj_encode(f)
    
    def encode(self, shared_features, location_xy):
        loc_xy_features = self.fc_location_xy(location_xy)
        concat_features = torch.cat((shared_features, loc_xy_features), dim=1)
    ...
    def forward(self, img, seg, location_xy):
        shared_features = self.estimate_shared_features(img, seg)
        mu, logvar = self.encode(shared_features, location_xy)
        z = self.reparameterize(mu, logvar)
        return self.decode(shared_features, z), mu, logvar
    
    @torch.no_grad()
    def predict(self, img, seg, z):
        shared_features = self.estimate_shared_features(img, seg)
