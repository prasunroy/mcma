...
# create model
model = ScalePredictor(embeds_dim, latent_dim, num_tposes)
...
def get_inputs(data):
    ...
    bb = data['bbox_params']
    if enable_gpu and torch.cuda.is_available():
        img, img_p1, img_p2, seg, seg_p1, seg_p2, em, bb = img.cuda(), img_p1.cuda(), img_p2.cuda(), seg.cuda(), seg_p1.cuda(), seg_p2.cuda(), em.cuda(), bb.cuda()
    return img, img_p1, img_p2, seg, seg_p1, seg_p2, em, bb
...
for epoch in range(num_epochs):
    ...
    for i, batch in enumerate(train_dataloader):
        t0 = time.time()
        img, img_p1, img_p2, seg, seg_p1, seg_p2, em, bb = get_inputs(batch)
        p, mu, logvar = model(img, img_p1, img_p2, seg, seg_p1, seg_p2, em, bb)
        loss_mse = torch.nn.functional.mse_loss(p, bb)
