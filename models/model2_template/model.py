...
class TemplatePredictor(nn.Module):
    ...
    def __init__(self, embeds_dim=512, num_classes=30):
        super(TemplatePredictor, self).__init__()
        ...
        self.fc_output = nn.Sequential(nn.Linear(embeds_dim*4*4, embeds_dim), nn.ReLU(inplace=True), nn.Linear(embeds_dim, num_classes))
        self.init_params()
    ...
    def forward(self, img, img_patch1, img_patch2, seg, seg_patch1, seg_patch2):
        ...
        return self.fc_output(fc)
    
    @torch.no_grad()
    def predict(self, img, img_patch1, img_patch2, seg, seg_patch1, seg_patch2):
        return self.forward(img, img_patch1, img_patch2, seg, seg_patch1, seg_patch2)
