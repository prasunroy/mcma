...
class DeformationPredictor(nn.Module):
    ...
    def __init__(self, embeds_dim=512, latent_dim=64, num_classes=30, num_keypoints=16):
        ...
        self.fc_deform = nn.Sequential(nn.Linear(2*num_keypoints, embeds_dim), nn.ReLU(inplace=True), nn.Linear(embeds_dim, embeds_dim), nn.ReLU(inplace=True))
        ...
        self.fc_output = nn.Sequential(nn.Linear(2*embeds_dim, embeds_dim), nn.ReLU(inplace=True), nn.Linear(embeds_dim, 2*num_keypoints))
        self.init_params()
    ...
    def encode(self, shared_features, deformations):
        deform_features = self.fc_deform(deformations)
    ...
    def forward(self, img, img_patch1, img_patch2, seg, seg_patch1, seg_patch2, pose_embedding, deformations):
        shared_features = self.estimate_shared_features(img, img_patch1, img_patch2, seg, seg_patch1, seg_patch2, pose_embedding)
        mu, logvar = self.encode(shared_features, deformations)
